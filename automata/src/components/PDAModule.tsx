...
interface PDAModuleProps {
  ...
  onSimulationComplete?: (rowId: number, isValid: boolean) => void; // ADDED CALLBACK
}
...
const PDAModule: React.FC<PDAModuleProps> = ({
  lastSimulated,
  selectedRegex,
  onSimulationComplete, // DESTRUCTURED CALLBACK
}) => {
  ...
  useEffect(() => {
    ...
    const simResult = runPDA(trimmed, selectedRegex);
    setResult(simResult);
    setShowTrace(false);

    // CALL THE PARENT COMPONENT TO UPDATE THE ROW STATUS
    if (onSimulationComplete) {
      onSimulationComplete(lastSimulated.rowId, simResult.accepted);
    }
  }, [lastSimulated, selectedRegex]); // Left out onSimulationComplete to avoid linting/render loops if unstable
